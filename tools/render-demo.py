...
WHEEL_PARTS = [bpy.data.objects[n] for n in ("wheel_body", "wheel_cap", "wheel_orange_part")
               if n in bpy.data.objects]
WHEEL_REST = [o.matrix_world.copy() for o in WHEEL_PARTS]
...
def set_wheel(degrees):
    """
    Spin the scroll wheel in the horizontal plane.

    About the world Z axis through the wheel's own centre, not about the
    object's local Z: the imported parts carry their own rotations, so adding to
    a local euler tips the dial out of plane instead of turning it.
    """
    rot = mathutils.Matrix.Rotation(math.radians(degrees), 4, "Z")
    for o, rest in zip(WHEEL_PARTS, WHEEL_REST):
        o.matrix_world = (
            mathutils.Matrix.Translation(WHEEL_CENTRE)
            @ rot
            @ mathutils.Matrix.Translation(-WHEEL_CENTRE)
            @ rest
        )
...
area("key", (0.40, -0.06, 0.30), (math.radians(38), 0, math.radians(8)), 0.75, 3.4)
area("fill", (0.30, 0.22, 0.14), (math.radians(64), 0, math.radians(-120)), 0.80, 1.5)
...
# A directory of bg_%03d.png, one per output frame, from tools/make-backdrop.py.
BACKDROP = arg("--backdrop", os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                          "..", ".js-build", "backdrop"))
BACKDROP_FRAMES = sorted(
    os.path.join(BACKDROP, f) for f in os.listdir(BACKDROP)
    if f.startswith("bg_") and f.endswith(".png")
) if os.path.isdir(BACKDROP) else []
backdrop_tex = None
if BACKDROP_FRAMES:
    ...
    btex.image = bpy.data.images.load(os.path.abspath(BACKDROP_FRAMES[0]))
...
# Shadows off engine-wide, not just per-light. Raytracing drives screen-space
# shadowing and ambient occlusion, which was still darkening the dial after the
# lights themselves stopped casting.
for attr, value in (("use_shadows", False), ("use_raytracing", False)):
    try:
...
# Expand holds into a flat list of output frames. Held beats used to be copied
# rather than re-rendered, which was free while the scene was static -- but the
# backdrop now moves every frame, so each one is its own render.
timeline = []
for entry in manifest:
    timeline.extend([entry] * max(1, int(entry.get("hold", 1))))
...
for n, entry in enumerate(timeline):
    ...
    if backdrop_tex is not None and BACKDROP_FRAMES:
        backdrop_tex.image.filepath = BACKDROP_FRAMES[n % len(BACKDROP_FRAMES)]
        backdrop_tex.image.reload()

    set_led(entry.get("led"))
    set_press(entry.get("press", 0))
    set_wheel(entry.get("wheel", 0))

    scene.render.filepath = os.path.join(OUT, f"r_{n:03d}.png")
    bpy.ops.render.render(write_still=True)
    print(f"rendered {n + 1}/{len(timeline)}", flush=True)
